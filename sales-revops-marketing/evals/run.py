...
# ///
"""Sales, RevOps & Marketing Eval Harness -- deterministic routing and content checks.
...
Usage:
    ...
    uv run evals/run.py --case route_prospect     # single case
...
# Paths
# ---------------------------------------------------------------------------
ROOT = Path(__file__).resolve().parent.parent  # sales-revops-marketing/ root
...
def main():
    parser = argparse.ArgumentParser(description="Sales, RevOps & Marketing Eval Harness")
    ...
    print(f"\nSales RevOps Marketing Eval Harness -- {len(cases)} cases", file=sys.stderr)
