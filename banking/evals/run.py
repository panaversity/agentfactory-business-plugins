...
# ///
"""Banking Eval Harness -- deterministic routing and content checks.
...
Usage:
    ...
    uv run evals/run.py --case route_ifrs9_ecl   # single case
...
# Paths
# ---------------------------------------------------------------------------
ROOT = Path(__file__).resolve().parent.parent  # banking/ root
...
def main():
    parser = argparse.ArgumentParser(description="Banking Eval Harness")
    ...
    # Run cases
    print(f"\nBanking Eval Harness -- {len(cases)} cases", file=sys.stderr)
