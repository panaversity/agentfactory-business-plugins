...
# ///
"""Operations Intelligence Eval Harness -- deterministic routing and content checks.
...
Usage:
    ...
    uv run evals/run.py --case route_audit_iso    # single case
...
# Paths
# ---------------------------------------------------------------------------
ROOT = Path(__file__).resolve().parent.parent  # operations-intelligence/ root
...
def main():
    parser = argparse.ArgumentParser(description="Operations Intelligence Eval Harness")
    ...
    # Run cases
    print(f"\nOperations Intelligence Eval Harness -- {len(cases)} cases", file=sys.stderr)
